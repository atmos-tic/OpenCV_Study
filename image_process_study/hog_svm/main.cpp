...
int main(int argc, const char* argv[])
{
 ...
 // cv::imshow("in", img); 
  ...
  cv::resize(frame, img, cv::Size(DSx, DSy));
  img.convertTo(dst, CV_32F, 1);
  ...
  for(int Sbx = 0; Sbx < DSx/Np-Nc+1; Sbx++){
    for(int Sby = 0; Sby < DSy/Np-Nc+1; Sby++){
      //各ブロック内の処理
      hog_hist[Sbx][Sby] = cv::Mat(Nc, Nc, CV_8UC(Nbin));//ヒストグラム初期化
      ...
      std::cout<<sum<<"x"<<Sbx<<"y"<<Sby<<std::endl;
      ...
      for (int i = 0; i < Nbin; i++) {
        ...
        cv::line(img, rp, lp, cv::Scalar(255*tmp.at<cv::Scalar>(Nc, Nc)[i], 255, 255)); 
      }     
    }
  }
  cv::imshow("img", img);
